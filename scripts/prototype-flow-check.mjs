...
add("导航包含五个市场模块入口", app.includes("市场") && app.includes("工作台") && app.includes("历史") && app.includes("额度") && app.includes("接入"));
...
add("市场模块交互存在", app.includes("应用市场") && app.includes("Agent 场景增值模块") && app.includes("返回主平台"));
add("默认入口切为应用市场", app.includes("MarketHero") && app.includes("按场景选择 Agent 应用") && !app.includes("统一 API 网关"));
add("嵌入模式保留市场状态条", app.includes("EmbeddedMarketHeader") && app.includes("剩余额度") && app.includes("模型分组"));
add("进入工作台仍保留模块能力", app.includes("market-mode") && app.includes("SideNav()") && app.includes("WorkspacePage"));
...
add("样式采用应用市场模块布局", css.includes(".market-topbar") && css.includes(".market-hero") && css.includes(".embedded-market-header") && css.includes(".work-grid"));
