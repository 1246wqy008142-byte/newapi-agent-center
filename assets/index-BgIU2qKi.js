(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))o(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const u of i.addedNodes)u.tagName==="LINK"&&u.rel==="modulepreload"&&o(u)}).observe(document,{childList:!0,subtree:!0});function s(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function o(n){if(n.ep)return;n.ep=!0;const i=s(n);fetch(n.href,i)}})();const x=document.querySelector("#app"),T=location.protocol==="file:",r=new URLSearchParams(location.search),g=r.get("embed")==="1"||r.get("mode")==="embed",v=r.get("dashboard")||"https://ai-dashboard.hkqlhnet.com",k=r.get("user")||"NewAPI 用户",D=r.get("group")||"默认分组",_=r.get("return_url")||v,f=location.pathname.includes("/newapi-agent-center")?"/newapi-agent-center":"",P=v.replace(/^https?:\/\//,"").replace(/\/$/,""),N="newapi-agent-center-v1",G=new Date().toISOString().slice(0,10),C=[{id:"all",label:"全部"},{id:"content",label:"内容运营"},{id:"sales",label:"客服销售"},{id:"office",label:"办公提效"},{id:"growth",label:"增长诊断"}],p=[{id:"writing",group:"content",title:"AI 写作工作台",audience:"自媒体、运营、商家",summary:"把主题、产品和目标人群转成小红书、公众号、短视频脚本和商品卖点。",cost:1200,status:"已上线",templates:[{id:"xiaohongshu",title:"小红书种草文案",fields:["产品/主题","目标人群","核心卖点","语气风格"],output:"生成标题、正文、标签和评论区引导。"},{id:"article",title:"公众号长文大纲",fields:["文章主题","读者画像","关键观点","期望长度"],output:"生成标题、大纲、开头、分段要点和结尾 CTA。"},{id:"video",title:"短视频脚本",fields:["视频主题","账号定位","转化目标","时长"],output:"生成开场钩子、分镜脚本、口播词和结尾引导。"}]},{id:"sales",group:"sales",title:"客服销售助手",audience:"私域商家、客服、销售",summary:"快速生成客户回复、价格异议处理、售前介绍和微信跟进话术。",cost:900,status:"已上线",templates:[{id:"reply",title:"客户问题回复",fields:["客户问题","产品信息","期望语气","是否需要促单"],output:"生成简洁可信的客服回复和下一步引导。"},{id:"objection",title:"价格异议处理",fields:["客户顾虑","产品优势","优惠规则","底线要求"],output:"生成不生硬的解释、价值重申和成交话术。"},{id:"followup",title:"微信跟进话术",fields:["客户阶段","上次沟通","本次目标","可用权益"],output:"生成三段不同强度的跟进话术。"}]},{id:"document",group:"office",title:"文件分析助手",audience:"职场、财务、法务、学生",summary:"先以文本粘贴版上线，后续接 PDF/Word/Excel 上传、解析和问答。",cost:1800,status:"二期增强",templates:[{id:"summary",title:"资料摘要",fields:["资料正文","阅读目标","输出格式","关注风险"],output:"生成重点摘要、待确认问题和行动清单。"},{id:"contract",title:"合同要点提取",fields:["合同文本","我方身份","关注条款","风险偏好"],output:"生成关键条款、风险提示和沟通建议。"}]},{id:"resume",group:"office",title:"简历面试助手",audience:"求职者、应届生、转行人群",summary:"根据岗位 JD 优化简历表达，预测面试问题并生成回答思路。",cost:1400,status:"已上线",templates:[{id:"optimize",title:"简历优化",fields:["目标岗位","简历内容","优势经历","希望突出"],output:"生成简历修改建议、项目表达和关键词补强。"},{id:"interview",title:"模拟面试题",fields:["目标岗位","公司行业","个人经历","薄弱环节"],output:"生成高频问题、回答框架和追问提醒。"}]},{id:"business",group:"growth",title:"生意诊断顾问",audience:"门店、小老板、独立创业者",summary:"输入行业、产品、客单价和目标，生成增长诊断、活动方案和内容策略。",cost:1600,status:"已上线",templates:[{id:"diagnosis",title:"门店增长诊断",fields:["行业/门店","目标客户","当前问题","预算范围"],output:"生成问题诊断、增长动作、执行节奏和指标。"},{id:"campaign",title:"促销活动方案",fields:["产品服务","活动目标","优惠空间","渠道"],output:"生成活动主题、权益组合、推广文案和复盘指标。"}]}],b={credits:Number(r.get("quota"))||86e3,usedToday:7200,selectedGroup:"all",selectedScenarioId:"writing",selectedTemplateId:"xiaohongshu",result:"",loading:!1,toast:"",formValues:{},history:[{id:"seed-1",date:"2026-07-28 20:16",scenario:"客服销售助手",template:"价格异议处理",tokens:940,title:"把“太贵了”转成价值沟通"},{id:"seed-2",date:"2026-07-27 11:42",scenario:"AI 写作工作台",template:"小红书种草文案",tokens:1260,title:"新品上线种草文案"}]};function R(){try{const t=JSON.parse(localStorage.getItem(N)||"{}");return{...b,...t,credits:r.has("quota")?Number(r.get("quota"))||b.credits:t.credits||b.credits}}catch{return b}}const e=R();function m(){localStorage.setItem(N,JSON.stringify({credits:e.credits,usedToday:e.usedToday,selectedGroup:e.selectedGroup,selectedScenarioId:e.selectedScenarioId,selectedTemplateId:e.selectedTemplateId,formValues:e.formValues,history:e.history}))}function y(){if(T){const a=decodeURIComponent(location.hash.replace(/^#/,""));return a.startsWith("/")?a:"/"}const t=location.pathname.startsWith(f)?location.pathname.slice(f.length)||"/":location.pathname;return t==="/"?"/":t}e.route=y();const E=[["/","⌂","应用"],["/workspace","✦","工作台"],["/history","▤","历史"],["/billing","◷","额度"],["/integration","↗","嵌入"]];function q(t){if(T)location.hash=t;else{const a=location.search||"";history.pushState({},"",`${f}${t}${a}`)}e.route=t,window.scrollTo({top:0,behavior:"smooth"}),c()}window.addEventListener("popstate",()=>{e.route=y(),c()});window.addEventListener("hashchange",()=>{e.route=y(),c()});function w(){return p.find(t=>t.id===e.selectedScenarioId)||p[0]}function L(){const t=w();return t.templates.find(a=>a.id===e.selectedTemplateId)||t.templates[0]}function H(){return e.selectedGroup==="all"?p:p.filter(t=>t.group===e.selectedGroup)}function l(t){return new Intl.NumberFormat("zh-CN").format(t)}function U(t){e.toast=t,c(),setTimeout(()=>{e.toast="",c()},1800)}function F(t){const a=p.find(s=>s.id===t);a&&(e.selectedScenarioId=a.id,e.selectedTemplateId=a.templates[0].id,m(),q("/workspace"))}function V(t){e.selectedTemplateId=t,e.result="",m(),c()}function h(t){return`<div class="app-shell ${g?"embedded":"module-mode"}">
    ${g?"":O()}
  ...
  </div>`}function O(){return`<div class="dashboard-topbar">
  ...
  </div>`}function W(){return`<header class="header">
    <div>
      <p class="eyebrow">AI Dashboard Add-on · ${G}</p>
      <h1>AI 应用中心</h1>
  ...
  </header>`}function B(){return`<nav class="side-nav">${E.map(([t,a,s])=>`
    <button class="${e.route===t?"active":""}" data-nav="${t}">
      <span>${a}</span>${s}
    </button>`).join("")}</nav>`}function J(){return g?"":`<nav class="mobile-nav">${E.map(([t,a,s])=>`
    <button class="${e.route===t?"active":""}" data-nav="${t}">
      <span>${a}</span><small>${s}</small>
    </button>`).join("")}</nav>`}function j(){return`<section class="stat-grid">
  ...
  </section>`}function M(){return`<section class="bridge-panel">
    <div>
  ...
  </section>`}function I(){return g?h(`<main class="content">
      ${M()}
      ${S()}
    ...
    <section class="scenario-grid">${H().map(z).join("")}</section>`}function z(t){return`<article class="scenario-card">
    <div class="card-top">
  ...
  </article>`}function K(){const t=w(),a=L();return h(`<main class="content workspace">
    ...
    <section class="work-grid">
      ...
      <section class="runner-panel">
        <div class="sync-strip"><span>用户：${d(k)}</span><span>模型分组：${d(D)}</span><span>日志：生成后同步</span></div>
        <form id="agent-form">
          <div class="form-head">
            <h3>${a.title}</h3>
            <span>${a.output}</span>
          </div>
          ${a.fields.map((s,o)=>Q(s,o)).join("")}
  ...
  </main>`)}function Q(t,a){const s=`${e.selectedScenarioId}:${e.selectedTemplateId}:${a}`,o=e.formValues[s]||"";return t.includes("正文")||t.includes("简历")||t.includes("合同")||t.includes("资料")?`<label class="field"><span>${t}</span><textarea name="${s}" placeholder="请输入${t}">${d(o)}</textarea></label>`:`<label class="field"><span>${t}</span><input name="${s}" value="${d(o)}" placeholder="请输入${t}"></label>`}function X(t){return d(t).replace(/\n/g,"<br>")}function d(t){return String(t).replace(/[&<>"']/g,a=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"})[a])}function Y(){return r.get("agent_endpoint")?r.get("agent_endpoint"):window.AGENT_AI_ENDPOINT?window.AGENT_AI_ENDPOINT:""}async function Z(t){const a=Y();if(!a)return null;const s=await fetch(a,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)});if(!s.ok)throw new Error(`Agent endpoint failed: ${s.status}`);const o=await s.json();return o.text||o.output_text||o.message||null}function A(t){const a=t.fields.filter(Boolean).join(" / ")||"用户输入信息";return[`【${t.templateTitle}】`,"","1. 任务判断",`围绕「${a}」优先输出可直接复制使用的内容，减少用户反复调 prompt 的成本。`,"","2. 推荐结果","建议采用“明确对象 + 关键卖点 + 行动引导”的结构，先给用户一个可用版本，再提供 2-3 个可继续优化的方向。","","3. 可继续优化","可以让用户选择语气、长度、平台风格、是否加强成交感，并在每次生成后记录 token 消耗和结果历史。","","4. 平台落点","本次结果适合作为增值场景的一次调用记录，后续可沉淀为模板、套餐或行业专属 Agent。"].join(`
`)}async function tt(t){if(e.loading)return;const a=new FormData(t);for(const[u,$]of a.entries())e.formValues[u]=$.toString();const s=w(),o=L(),n=o.fields.map((u,$)=>a.get(`${s.id}:${o.id}:${$}`)?.toString().trim()||"");e.loading=!0,e.result="",m(),c();const i={scenarioId:s.id,scenarioTitle:s.title,templateId:o.id,templateTitle:o.title,fields:n,system:"你是 NewAPI token 平台的场景 Agent。请把用户输入转成可直接使用的业务结果，输出中文，结构清晰，适合网页应用展示。"};try{e.result=await Z(i)||A(i)}catch{e.result=A(i)}e.loading=!1,e.credits=Math.max(0,e.credits-s.cost),e.usedToday+=s.cost,e.history.unshift({id:`run-${Date.now()}`,date:new Date().toLocaleString("zh-CN",{hour12:!1}),scenario:s.title,template:o.title,tokens:s.cost,title:n.find(Boolean)||o.title}),m(),c()}function et(){return h(`<main class="content">
  ...
  </main>`)}function at(){return h(`<main class="content">
    <section class="page-title">
  ...
  </main>`)}function st(){const t=location.origin==="null"?"https://agent.yourdomain.com":location.origin,s=`${t}${f||""}/?embed=1&user={{user_id}}&quota={{quota}}&group={{group}}&return_url=${encodeURIComponent(v)}`;return h(`<main class="content">
    <section class="page-title">
  ...
  </main>`)}function nt(){return({"/":I,"/workspace":K,"/history":et,"/billing":at,"/integration":st}[e.route]||I)()}function ot(){document.querySelectorAll("[data-nav]").forEach(t=>t.addEventListener("click",()=>q(t.dataset.nav))),document.querySelectorAll("[data-group]").forEach(t=>t.addEventListener("click",()=>{e.selectedGroup=t.dataset.group,m(),c()})),document.querySelectorAll("[data-scenario]").forEach(t=>t.addEventListener("click",()=>F(t.dataset.scenario))),document.querySelectorAll("[data-template]").forEach(t=>t.addEventListener("click",()=>V(t.dataset.template))),document.querySelector("#agent-form")?.addEventListener("submit",t=>{t.preventDefault(),tt(t.currentTarget)}),document.querySelector("[data-clear]")?.addEventListener("click",()=>{Object.keys(e.formValues).forEach(t=>{t.startsWith(`${e.selectedScenarioId}:${e.selectedTemplateId}:`)&&delete e.formValues[t]}),e.result="",m(),c()}),document.querySelector("[data-copy]")?.addEventListener("click",async()=>{await navigator.clipboard.writeText(e.result),U("结果已复制")}),document.querySelectorAll("[data-open-dashboard]").forEach(t=>t.addEventListener("click",()=>{window.open(_,"_blank","noopener,noreferrer")}))}function c(){x.innerHTML=nt(),ot()}c();
